...
from dml4fluxes.models.nn_regressor import EnsembleCustomJNN
...
class dml_fluxes():
    ...
    def fit(self, Y, X, T, W=None, X_reco=None):
        # Build model selection functionality in the future
    ...
    def get_estimators(self):
        ...
        def reco(x, t=None, w=None, y=None, ta=None):
            return np.mean([
                self.est._models_nuisance[0][i]._model_y.predict(x, w) 
                - self.est.const_marginal_effect(x) 
                * (self.est._models_nuisance[0][i]._model_t.predict(x, w))
                for i in range(len(self.est._models_nuisance[0]))
            ], axis=0)
