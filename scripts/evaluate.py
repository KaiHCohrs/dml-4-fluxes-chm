...
def process_row(site, start_year, end_year, data_folder, result_folder, dml_suffix):
    results = []
    for year in range(start_year, end_year + 1):
        ...
        for column in columns:
            for suffix, quality, partial in product(["_DT", "_NT"], ["","_Q"], ["","_D", "_N"]):
                if column == "RECO":
                    column_name = f"{column}_{dml_suffix}_0"
                elif (column == "NEE") and (dml_suffix == "fit"):
                    column_name = f"{column}_{dml_suffix}_0"
                else:
                    column_name = f"{column}_0"
                ...
                # isnan mask
                nan_mask = ~np.isnan(predicted[column_name]) & ~np.isnan(data[column + suffix])


                true_values = data[column + suffix][partial_mask & quality_mask & nan_mask]
                predicted_values = predicted[column_name][partial_mask & quality_mask & nan_mask]
...
def main(input_file, data_folder, result_folder, dml_suffix):
    ...
    for index, row in sites_data.iterrows():
        ...
        results = process_row(site, start_year, end_year, data_folder, result_folder, dml_suffix)
    ...
    # Save results to CSV
    results_df.to_csv(Path(result_folder).joinpath(f"results_{dml_suffix}.csv"), index=False)
    summary_stats = []
    ...
    summary_stats.to_csv(Path(result_folder).joinpath(f"summary_stats_{dml_suffix}.csv"))
...
if __name__ == "__main__":
    ...
    result_folder = "~/Repositories/dml-4-fluxes/results/ngoc_dry_dml_default_single"

    main(input_file, data_folder, result_folder, dml_suffix)